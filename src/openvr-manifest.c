...
typedef struct
{
  OpenVRBindingComponent component;
  gchar *input_path;
} OpenVRBindingInputPath;

typedef struct
{
  OpenVRBindingType binding_type;
  GList *input_paths;
  OpenVRBindingMode mode;
} OpenVRBinding;

struct _OpenVRManifest
{
  GObject parent;
...
G_DEFINE_TYPE (OpenVRManifest, openvr_manifest, G_TYPE_OBJECT)

static void
openvr_manifest_finalize (GObject *gobject);

static void
openvr_manifest_class_init (OpenVRManifestClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->finalize = openvr_manifest_finalize;
}

static void
_free_input_path (gpointer data)
{
  OpenVRBindingInputPath *path = data;
...
static gboolean
_parse_actions (OpenVRManifest *self, GInputStream *stream)
{
  GError *error = NULL;
  ...
  for (guint i = 0; i < len; i++)
    {
      ...
      OpenVRBinding *binding = g_malloc (sizeof (OpenVRBinding));
...
static gboolean
_parse_bindings (OpenVRManifest *self, GInputStream *stream)
{
  GError *error = NULL;
          ...
          for (GList *m = input_list; m != NULL; m = m->next)
            {
              ...
              OpenVRBinding *binding = g_hash_table_lookup (self->actions, output);
              if (!binding)
                {
              ...
              OpenVRBindingInputPath *input_path = g_malloc (sizeof (OpenVRBindingInputPath));
...
OpenVRManifest *
openvr_manifest_new (void)
{
  return (OpenVRManifest *) g_object_new (OPENVR_TYPE_MANIFEST, 0);
}

static void
openvr_manifest_finalize (GObject *gobject)
{
  OpenVRManifest *self = OPENVR_MANIFEST (gobject);
